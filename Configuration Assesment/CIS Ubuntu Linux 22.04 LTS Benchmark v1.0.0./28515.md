...
3. **Copy Existing `/var/log/audit` Data:**
   ...
   * Verify sizes:
     ...
     ```bash
     sudo du -sh /mnt/ubuntu_root/var/log/audit
     sudo du -sh /mnt/new_varlogaudit
     ```
...
5. **Edit `fstab` in the Installed System:**
   ...
   * Add a line for `/var/log/audit`. You can choose:
     ...
     * **UUID entry** (robust if device names change):
       ...
       ```
       UUID=<UUID_OF_NEW_VARLOGAUDIT>    /var/log/audit    ext4    defaults,nosuid,nodev,noexec    0   0
       ```
     * **Direct device path** (simpler but may break if naming shifts):

       ```
       /dev/<NEW_VARLOGAUDIT_PARTITION>    /var/log/audit    ext4    defaults,nosuid,nodev,noexec    0   0
       ```
   * **Reasoning**:
