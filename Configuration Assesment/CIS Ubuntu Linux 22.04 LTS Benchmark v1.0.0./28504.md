...
#### b. Backup Existing `/var` Data (Optional: from Installed System)

It’s good practice to back up `/var` before partition changes. You can back up while booted normally or wait until the Live USB session. Examples below assume you have an external drive or network location mounted at `<BACKUP_DESTINATION>`.

1. Ensure `<BACKUP_DESTINATION>` is accessible and has enough free space.
2. Run:

   ```bash
   sudo rsync -avh --progress /var/ <BACKUP_DESTINATION>/var_backup_$(date +%F)/
   ```
3. Verify the backup:

   ```bash
   du -sh <BACKUP_DESTINATION>/var_backup_$(date +%F)/
   ```

   Ensure it matches or exceeds the size shown by `du -sh /var`.

> **Tip:** If you prefer, you can also perform this backup in the Live USB environment after mounting the root partition (see next section).

---

### 2. Boot Live USB and (Re-)Backup `/var` from Live Environment
...
2. **(Optional) Mount the Ubuntu Root and Backup `/var`:**
   If you did not back up already, you can back up now from Live:

   * Identify your root partition (e.g., via `lsblk` or `sudo fdisk -l`)—we’ll refer to it as `<ROOT_PARTITION>` (e.g., `/dev/sdXN`).
   * Mount it:
     ...
     ```
   * Mount or ensure access to your backup destination (`<BACKUP_DESTINATION>`), e.g., an external USB drive might auto-mount under `/media/ubuntu/...`.
   * Run:

     ```bash
     sudo rsync -avh --progress /mnt/ubuntu_root/var/ <BACKUP_DESTINATION>/var_backup_$(date +%F)/
     ```
   * Verify sizes:
